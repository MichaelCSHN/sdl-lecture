...
import { Play, RotateCcw, Settings, ChevronDown, ChevronRight } from 'lucide-react';
import {
  GaussianProcess,
  expectedImprovement,
  setBORngSeed,
...
function probImprovement(mu: number, sigma: number, yBest: number, xi: number = 0.01): number {
...
const TABS: { id: TabId; label: string; desc: string }[] = [
  { id: '1d', label: '1D 单目标', desc: 'Forrester — surrogate / uncertainty / acquisition' },
  { id: '2d', label: '2D 单目标', desc: 'Branin-Hoo — 全局搜索 / explore vs exploit' },
...
export default function SDLDemoPage() {
  ...
  const doReset = useCallback(() => {
    ...
    if (tab === 'multi') {
      ...
      initY = initMOObs.map((obs) => {
        const { scalarized } = scalarizeRandom(obs, seed + initX.indexOf(obs));
        return scalarized;
      });
    } else {
    ...
    const bestIdx = initY.reduce((best, _, i) =>
      (optimDir === 'min' ? initY[i] < initY[best] : initY[i] > initY[best]) ? i : best, 0);
    ...
    computeVisData(state, gp, tab, acqFn, optimDir, ucbBeta, domain, seed);
    setRunState({ ...state });
  }, [seed, tab, acqFn, optimDir, nInit, initDesign, noise, ucbBeta]);
  ...
  const runStep = useCallback(() => {
    ...
    // Evaluate all candidates for acquisition
    let bestAcqVal = optimDir === 'min' ? Infinity : -Infinity;
    let bestCandidate = candidates[0];
    let bestMean = 0, bestStd = 0;
    const yBest = runState.y.reduce((b, y, i) =>
      (optimDir === 'min' ? y < runState.y[b] : y > runState.y[b]) ? i : b, 0);
    const bestY = runState.y[yBest];

    for (const cand of candidates) {
      const pred = gp.predict(cand);
      let acqVal: number;
      switch (acqFn) {
        case 'EI': acqVal = expectedImprovement(pred.mean, pred.std, bestY); break;
        case 'UCB': acqVal = optimDir === 'min' ? -(pred.mean - ucbBeta * pred.std) : pred.mean + ucbBeta * pred.std; break;
        case 'PI': acqVal = probImprovement(pred.mean, pred.std, bestY); break;
        default: acqVal = uniformBORandom(); break;
      }
      const isBetter = optimDir === 'min' ? acqVal < bestAcqVal : acqVal > bestAcqVal;
      if (isBetter) {
        bestAcqVal = acqVal;
        bestCandidate = cand;
        bestMean = pred.mean;
    ...
    let obs: number;
    if (tab === 'multi') {
      const obj1 = braninCurrinObj1(bestCandidate[0], bestCandidate[1]);
      const obj2 = braninCurrinObj2(bestCandidate[0], bestCandidate[1]);
      const { scalarized } = scalarizeRandom([obj1, obj2], seed + runState.iteration);
      obs = scalarized;
      const newMOObs = [...(runState.moObs || []), [obj1, obj2]];
      runState.moObs = newMOObs;
      runState.paretoPoints = paretoFront(
        runState.X.map((x, i) => ({ x, objectives: newMOObs[i] }))
      );
      const pf = runState.paretoPoints.map((p) => p.objectives);
      runState.hvHistory.push(hypervolume2D(pf, [400, 20]));
    } else {
      const fn = tab === '1d' ? (x: number[]) => forrester(x[0]) : (x: number[]) => branin(x[0], x[1]);
      obs = fn(bestCandidate) + (noise > 0 ? gaussianRandom(0, noise) : 0);
    }

    runState.X.push(bestCandidate);
    runState.y.push(obs);
    runState.iteration++;

    const newBest = optimDir === 'min'
      ? Math.min(runState.bestY, obs)
      : Math.max(runState.bestY, obs);
    if (newBest !== runState.bestY) {
      runState.bestY = obs;
      runState.bestX = bestCandidate;
    }

    runState.nextX = bestCandidate;
    runState.nextReason = buildReason(acqFn, bestMean, bestStd, bestY, runState.iteration);
    gp.fit(runState.X, runState.y);
    computeVisData(runState, gp, tab, acqFn, optimDir, ucbBeta, domain, seed);
    setRunState({ ...runState });
  }, [runState, tab, acqFn, optimDir, seed, ucbBeta, noise]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div className="lg:col-span-1 space-y-3">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <div className="flex-1">
                  <label className="text-[9px] text-[#8a92a3]">方向</label>
              ...
              <div className="flex gap-2">
                ...
                <div className="flex-1">
                  ...
                  <input type="number" value={seed} onChange={(e) => setSeed(Number(e.target.value))} min={1} max={9999}
                    className="w-full mt-1 px-2 py-1 rounded text-[10px] bg-[rgba(67,97,238,0.08)] text-[#d0d4dc] border border-[rgba(67,97,238,0.15)]" />
                </div>
              </div>
          ...
          <div className="flex flex-wrap gap-2">
            <button onClick={() => { doReset(); }} className="flex items-center gap-1 px-3 py-1.5 rounded border border-[rgba(255,107,107,0.2)] text-[#ff6b6b] text-[10px] font-mono hover:bg-[rgba(255,107,107,0.06)]">
              <RotateCcw className="w-3 h-3" /> 初始化/重置
            </button>
            <button onClick={runStep} disabled={!runState} className="flex items-center gap-1 px-3 py-1.5 rounded border border-[rgba(0,245,212,0.3)] text-[#00f5d4] text-[10px] font-mono hover:bg-[rgba(0,245,212,0.06)] disabled:opacity-40">
        ...
        <div className="lg:col-span-3">
          {!runState ? (
            <div className="flex items-center justify-center h-80 border border-dashed border-[rgba(67,97,238,0.15)] rounded-lg text-[10px] text-[#8a92a3]">
              点击「初始化/重置」加载 benchmark，然后「运行 1 步」观察 BO 迭代
            </div>
          ) : (
            <Suspense fallback={<div className="h-80 flex items-center justify-center text-[10px] text-[#8a92a3]">加载图表…</div>}>
              <PlotsView tab={tab} runState={runState} _optimDir={optimDir} acqFn={acqFn} />
            </Suspense>
          )}
        </div>
      </div>
...
// ================================================================
// Plot rendering — separated for Suspense
// ================================================================

function PlotsView({ tab, runState, _optimDir, acqFn }: { tab: TabId; runState: RunState; _optimDir: OptimDir; acqFn: AcqFn }) {
  if (tab === '1d' && runState.gpData) {
    return <OneDPlots runState={runState} />;
  }
  if (tab === '2d' && runState.contData) {
    return <TwoDPlots runState={runState} _optimDir={_optimDir} />;
  }
  if (tab === 'multi') {
    return <MultiObjPlots runState={runState} acqFn={acqFn} />;
  }
...
function OneDPlots({ runState }: { runState: RunState }) {
  ...
  const trueTrace = {
    x: xs, y: gd.x.map((_, i) => forrester(gd.x[i][0])),
    type: 'scatter' as const, mode: 'lines' as const,
    name: '真实函数', line: { color: '#00f5d4', width: 2 },
  };
  const meanTrace = {
    x: xs, y: gd.mean,
    type: 'scatter' as const, mode: 'lines' as const,
    name: 'GP 均值', line: { color: '#4361ee', width: 1.5, dash: 'dash' as const },
  };
  const ciFill = {
    x: [...xs, ...xs.reverse()],
    y: [...gd.mean.map((m, i) => m + 2 * gd.std[i]), ...gd.mean.map((m, i) => m - 2 * gd.std[i]).reverse()],
    type: 'scatter' as const, mode: 'lines' as const, fill: 'toself' as const,
    fillcolor: 'rgba(67,97,238,0.12)', line: { width: 0 },
    name: '95% 置信', showlegend: true,
  };
  const sampTrace = {
    x: runState.X.map((x) => x[0]), y: runState.y,
    type: 'scatter' as const, mode: 'markers' as const,
    name: '采样点', marker: { color: '#fee440', size: 8, symbol: 'circle-open' },
  };
  const nextTrace = runState.nextX ? {
    x: [runState.nextX[0]], y: [0], // y is ignored — drawn at bottom
    type: 'scatter' as const, mode: 'markers' as const,
    name: '下一推荐', marker: { color: '#ff6b6b', size: 12, symbol: 'triangle-down' },
  } : null;

  const mainData = [trueTrace, meanTrace, ciFill, sampTrace, nextTrace].filter(Boolean) as any[];
  const mainLayout = {
    title: { text: '代理模型与采样', font: { color: '#d0d4dc', size: 12 } },
    ...
    height: 300, legend: { x: 0.01, y: 0.99, font: { size: 9 } },
    showlegend: true,
  };
  ...
  const acqTrace = {
    x: xs, y: ad.y,
    type: 'scatter' as const, mode: 'lines' as const,
    name: '采集函数', line: { color: '#f59e0b', width: 1.5 },
  };
  const acqLayout = {
    title: { text: '采集函数', font: { color: '#d0d4dc', size: 12 } },
    xaxis: { title: { text: 'x', font: { color: '#8a92a3' } }, gridcolor: 'rgba(67,97,238,0.08)', color: '#8a92a3' },
  ...
  return (
    <div>
      <Plot data={mainData} layout={mainLayout} useResizeHandler style={{ width: '100%' }} />
...
function TwoDPlots({ runState, _optimDir }: { runState: RunState; _optimDir: OptimDir }) {
  ...
  const contTrace = {
    ...
    type: 'contour' as const, name: 'Branin',
    colorscale: [[0, '#06162a'], [0.4, '#4361ee'], [0.7, '#00f5d4'], [1, '#fee440']],
    contours: { coloring: 'heatmap' as const },
    showscale: false,
  };
  const sampTrace = {
    x: runState.X.map((x) => x[0]), y: runState.X.map((x) => x[1]),
    type: 'scatter' as const, mode: 'markers+lines' as const,
    name: '采样轨迹', marker: { color: runState.X.map((_, i) => i === runState.X.length - 1 ? '#ff6b6b' : '#fee440'), size: 6 },
    line: { color: 'rgba(254,68,0,0.4)', width: 1 },
  };
  const bestTrace = {
    x: [runState.bestX[0]], y: [runState.bestX[1]],
    type: 'scatter' as const, mode: 'markers' as const,
    name: '当前最佳', marker: { color: '#00f5d4', size: 10, symbol: 'star' },
  };
  const nextTrace = runState.nextX ? {
    x: [runState.nextX[0]], y: [runState.nextX[1]],
    type: 'scatter' as const, mode: 'markers' as const,
    name: '下一推荐', marker: { color: '#ff6b6b', size: 10, symbol: 'x' },
  } : null;

  const data = [contTrace, sampTrace, bestTrace, nextTrace].filter(Boolean) as any[];
  const layout = {
    title: { text: 'Branin-Hoo — 搜索轨迹', font: { color: '#d0d4dc', size: 12 } },
    xaxis: { title: { text: 'x₁', font: { color: '#8a92a3' } }, range: [-5, 10], gridcolor: 'rgba(67,97,238,0.08)', color: '#8a92a3' },
    yaxis: { title: { text: 'x₂', font: { color: '#8a92a3' } }, range: [0, 15], gridcolor: 'rgba(67,97,238,0.08)', color: '#8a92a3' },
    paper_bgcolor: 'transparent', plot_bgcolor: 'rgba(0,13,29,0.5)',
    font: { color: '#8a92a3', size: 10 },
    margin: { t: 30, r: 10, b: 40, l: 50 }, height: 360,
    legend: { x: 0.01, y: 0.99, font: { size: 9 } },
  };

  const bestYData = runState.y.map((_, i) => {
    const slice = runState.y.slice(0, i + 1);
    return optimDir === 'min' ? Math.min(...slice) : Math.max(...slice);
  });
  const bestTrace2 = {
    x: bestYData.map((_, i) => i + 1), y: bestYData,
    type: 'scatter' as const, mode: 'lines+markers' as const,
    name: 'best-so-far', line: { color: '#00f5d4', width: 1.5 },
    marker: { size: 4, color: '#00f5d4' },
  };
  const bestLayout = {
    title: { text: 'Best-so-far vs 迭代', font: { color: '#d0d4dc', size: 12 } },
  ...
  return (
    <div>
      ...
      <Plot data={[bestTrace2]} layout={bestLayout} useResizeHandler style={{ width: '100%' }} />
    </div>
  );
}
...
function MultiObjPlots({ runState, acqFn }: { runState: RunState; acqFn: AcqFn }) {
  ...
  const scatterTrace = {
    x: objPoints.map((o) => o[0]), y: objPoints.map((o) => o[1]),
    type: 'scatter' as const, mode: 'markers' as const,
    name: '所有观测', marker: { color: '#8a92a3', size: 5, opacity: 0.6 },
  };
  const pfTrace = pfObjs.length > 0 ? {
    x: pfObjs.map((o) => o[0]).sort((a, b) => a - b),
    y: pfObjs.map((o) => o[1]).sort((a, b) => a - b),
    type: 'scatter' as const, mode: 'lines+markers' as const,
    name: 'Pareto front', line: { color: '#00f5d4', width: 2 },
    marker: { size: 6, color: '#00f5d4' },
  } : null;
  const lastIdx = objPoints.length - 1;
  const recTrace = lastIdx >= 0 ? {
    x: [objPoints[lastIdx][0]], y: [objPoints[lastIdx][1]],
    type: 'scatter' as const, mode: 'markers' as const,
    name: '最新推荐', marker: { color: '#ff6b6b', size: 10, symbol: 'x' },
  } : null;

  const objData = [scatterTrace, pfTrace, recTrace].filter(Boolean) as any[];
  const objLayout = {
    title: { text: '目标空间 — Pareto Front', font: { color: '#d0d4dc', size: 12 } },
    xaxis: { title: { text: 'Branin (越小越好)', font: { color: '#8a92a3' } }, gridcolor: 'rgba(67,97,238,0.08)', color: '#8a92a3' },
    yaxis: { title: { text: 'Currin (越小越好)', font: { color: '#8a92a3' } }, gridcolor: 'rgba(67,97,238,0.08)', color: '#8a92a3' },
    paper_bgcolor: 'transparent', plot_bgcolor: 'rgba(0,13,29,0.5)',
    font: { color: '#8a92a3', size: 10 },
  ...
  const hvTrace = runState.hvHistory.length > 0 ? {
    x: runState.hvHistory.map((_, i) => runState.X.length - runState.hvHistory.length + i + 1),
    y: runState.hvHistory,
    type: 'scatter' as const, mode: 'lines+markers' as const,
    name: 'Hypervolume', line: { color: '#00f5d4', width: 1.5 },
    marker: { size: 4, color: '#00f5d4' },
  } : null;
  const hvLayout = {
    title: { text: 'Hypervolume vs 迭代', font: { color: '#d0d4dc', size: 12 } },
    xaxis: { title: { text: '迭代', font: { color: '#8a92a3' } }, gridcolor: 'rgba(67,97,238,0.08)', color: '#8a92a3' },
    yaxis: { gridcolor: 'rgba(67,97,238,0.08)', color: '#8a92a3' },
    paper_bgcolor: 'transparent', plot_bgcolor: 'rgba(0,13,29,0.5)',
    font: { color: '#8a92a3', size: 10 },
  ...
  return (
    <div>
      ...
      <div className="p-3 mt-2 rounded border border-[rgba(67,97,238,0.1)] text-[10px] text-[#8a92a3] leading-relaxed">
        <strong className="text-[#d0d4dc]">当前方法：</strong>ParEGO 风格的随机标量化（Augmented Tchebycheff）。
        每次迭代随机生成权重 → 标量化为单目标 → 单目标 BO（{acqFn}）→ 更新 Pareto 集。
        这不是真正的多目标 BO，但在教学中能让学生追踪标量化→单目标推荐→Pareto 改善的逻辑链。
      </div>
    </div>
  );
}
...
function computeVisData(
  ...
  if (tab === '1d') {
    ...
    const xs: number[][] = [];
    for (let i = 0; i < n; i++) {
      xs.push([domain[0][0] + (i / (n - 1)) * (domain[0][1] - domain[0][0])]);
    }
    const means: number[] = [];
    const stds: number[] = [];
    ...
    for (const x of xs) {
      ...
      let av: number;
      switch (acqFn) {
        case 'EI': av = expectedImprovement(p.mean, p.std, yBest); break;
        case 'UCB': av = optimDir === 'min' ? -(p.mean - ucbBeta * p.std) : p.mean + ucbBeta * p.std; break;
        case 'PI': av = probImprovement(p.mean, p.std, yBest); break;
...
function buildReason(acqFn: AcqFn, mean: number, std: number, bestY: number, iter: number): string {
  if (iter <= 3) return `[初始探索] 仅有少量采样点，GP 的 uncertainty 较高。推荐点基于先验分布和采集函数 ${acqFn} 选择。`;
  if (acqFn === 'EI' && mean < bestY) return `[利用] EI 发现该区域预测值 (${mean.toFixed(3)}) 优于当前最佳 (${bestY.toFixed(3)})，且不确定性较低，值得在此"精炼"搜索。`;
  if (acqFn === 'UCB' && std > 2) return `[探索] UCB 偏向高 uncertainty 区域 (σ=${std.toFixed(2)})。GP 对这片区域了解不足，模型主动"试探"。`;
  return `[平衡] ${acqFn} 在预测值 (${mean.toFixed(3)}) 和 uncertainty (σ=${std.toFixed(2)}) 之间权衡。`;
}
