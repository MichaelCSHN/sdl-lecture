...
const studioFields = [
  { name: '研究问题', nameEn: 'Research Question',
    prompt: '用一句不带"和"的话写清你的问题，明确你想解释、比较或优化的对象。',
    placeholder: '例：在 300–600°C 范围内，沉积温度如何影响 TiO₂ 薄膜的光催化活性？' },
  { name: '目标与结果变量', nameEn: 'Objective & Output Variable',
    prompt: '说明主要结果变量是什么，用什么仪器、什么单位、什么精度测量。',
    placeholder: '例：光催化降解率（%），UV-Vis 分光光度计，精度 ±0.5%，目标：最大化。' },
  { name: '因素与约束', nameEn: 'Factors & Constraints',
    prompt: '列出你真正打算操控的因素、必须固定的变量，以及不能突破的安全或成本边界。',
    placeholder: '例：自变量：沉积温度（300–600°C），气压（0.1–10 Pa）；固定：基底材料；约束：粗糙度 < 5 nm。' },
  { name: '实验设计方案', nameEn: 'Experimental Design',
    prompt: '解释为什么选择 OFAT、析因设计、RSM，还是贝叶斯优化 / SDL 闭环？说明实验次数估计。',
    placeholder: '例：参数连续且昂贵，选贝叶斯优化。初始 5 个 seed 点，EI 策略，预计 20 次内收敛。' },
  { name: '执行与记录', nameEn: 'Execution & Record-keeping',
    prompt: '谁来随机化、谁来盲标、如何记录原始值、如何保证 SOP 被真正执行。',
    placeholder: '例：实验顺序随机化（Python random.shuffle），原始数据记录在 ELN，每批次附仪器校准记录。' },
  { name: '数据与分析计划', nameEn: 'Data & Analysis Plan',
    prompt: '异常值处理规则、统计检验、敏感性分析——要在数据采集前就写明，不能在结果出来后倒推。',
    placeholder: '例：3σ 原则剔除异常值；GP 模型更新后检查后验覆盖度；用独立验证集验证最优点。' },
  { name: '风险与验证', nameEn: 'Risks & Validation',
    prompt: '指出最可能失败的步骤，并说明你如何用预实验、对照或独立重复验证关键结论。',
    placeholder: '例：最大风险：高温导致薄膜脱落（T > 560°C），用预实验确定失效边界，最优点做三次重复验证。' },
  { name: '哪些判断必须由人完成', nameEn: 'Required Human Judgment',
    prompt: '明确哪些环节可以交给自动化，哪些环节仍需要研究者做机制判断与边界设定。',
    placeholder: '例：参数边界定义、失败判据认定、最终相结构解读均不能委托给算法。算法推荐，人负责验证。' },
];
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="flex items-start justify-between gap-6 flex-wrap">
          <div>
            ...
            <p className="text-[#8a92a3] max-w-3xl leading-7 text-sm">
              承接 8 步实验工作流。目标不是"写得漂亮"，而是让你真正暴露问题：
              哪里还没有变量定义，哪里还没有对照，哪里只是口号式地说"想用 SDL"。
            </p>
          </div>
          <div className="flex items-center gap-3 shrink-0">
            <span className="text-xs font-mono text-[#8a92a3]">{filledCount}/{studioFields.length} 已填</span>
            <button onClick={copyAll} disabled={filledCount === 0}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded border text-[11px] font-mono transition-colors
                         border-[rgba(0,245,212,0.25)] text-[#00f5d4] hover:bg-[rgba(0,245,212,0.08)]
                         disabled:opacity-30 disabled:cursor-not-allowed">
              {copied ? <CheckCheck className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
              {copied ? '已复制' : '复制全文'}
            </button>
            <button onClick={clearAll}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded border text-[11px] font-mono
                         border-[rgba(255,107,107,0.2)] text-[#ff6b6b] hover:bg-[rgba(255,107,107,0.06)]">
      ...
      <div className="h-1 rounded-full bg-[rgba(67,97,238,0.12)] mb-8 overflow-hidden">
        <div className="h-full rounded-full bg-[#00f5d4] transition-all duration-500"
          style={{ width: `${(filledCount / studioFields.length) * 100}%` }} />
      </div>
      ...
      <div className="grid gap-5 md:grid-cols-2 mb-10">
        {studioFields.map((field, index) => {
          ...
          return (
            <div key={field.name} className="glass-panel rounded-2xl border p-5 transition-colors"
              style={{ borderColor: filled ? 'rgba(0,245,212,0.25)' : 'rgba(67,97,238,0.12)' }}>
              <div className="flex items-center gap-3 mb-2">
                <span className="w-7 h-7 rounded-lg border flex items-center justify-center text-[10px] font-mono flex-shrink-0 transition-colors"
                  style={{
              ...
              <textarea
                ...
                rows={4}
                className="w-full rounded-xl border bg-[rgba(255,255,255,0.02)] px-4 py-3
                           text-sm text-[#d0d4dc] placeholder-[#3a3f4a] resize-none leading-7
                ...
                onBlur={(e) => { e.target.style.borderColor = filled ? 'rgba(0,245,212,0.2)' : 'rgba(67,97,238,0.15)'; }}
              />
            </div>
          );
        })}
      ...
      <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <div className="glass-panel rounded-2xl border border-[rgba(0,245,212,0.14)] p-5">
          ...
          <div className="space-y-3">
            {[
              { to: '/methods', title: '回到 8 步实验工作流', desc: '如果你发现自己答不上来，回到完整流程页面检查到底卡在哪一步。' },
              { to: '/foundations#ofat-to-sdl', title: '重看 DOE 与 SDL 的边界', desc: '很多课题不是"缺一个算法"，而是还没完成实验设计与测量框架的收束。' },
              { to: '/resources', title: '进入阅读轨与资源库', desc: '如果你想补某一段方法论背景，这页已经按主题和阶段整理好了入口。' },
            ].map((link) => (
